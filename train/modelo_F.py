...
PATH = f'../data/processed/F/{HORAS_PASADAS}_{HORAS_POR_PREDECIR}_entrenamiento.pickle'

file_exists = exists(PATH)

if not file_exists:
    import data.procesamiento_datos_F
...
sv = list(range(200,1500,200))
sv += [100,150,1500]
nbt = int(len(datos)/64)
error = []
...
for i in range(epochs):
    ...
    error.append(sum(flow[-nbt:])/nbt)
    print(f'Epoca {i+1} - error: {error[-1]}')


    if i in sv:
        torch.save(GCLSTM_DECODER.state_dict(), f'../mod_entr/GCLSTM_DECODER_{i}_epocas.pt')
        torch.save(GCLSTM_ENCODER.state_dict(), f'../mod_entr/GCLSTM_ENCODER_{i}_epocas.pt')
        torch.save(MLP_OUT.state_dict(), f'../mod_entr/MLP_OUT_{i}_epocas.pt')

        with open(f'../mod_entr/Laplaciano_{i}_epocas.pickle', 'wb') as handle:
            pickle.dump(GCLSTM.Lap_nonze, handle, protocol=pickle.HIGHEST_PROTOCOL)

pickle.dump(flow, open('../mod_entr/lss_F.pkl', 'wb'))
